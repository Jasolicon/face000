...
from train_transformer3D.models_3d import PoseConditionedAttention
...
if __name__ == "__main__":
    # 测试模型
    print("=" * 70)
    print("测试 TransformerDecoderOnly3D_Triplet 模型")
    print("=" * 70)
    
    # 创建模型
    print("\n创建 TransformerDecoderOnly3D_Triplet 模型...")
    ...
    print(f"身份特征范围: [{identity_features.min().item():.4f}, {identity_features.max().item():.4f}]")
    print(f"身份特征L2范数（应该接近1.0）: {torch.norm(identity_features, dim=1).mean().item():.4f}")
    print(f"残差: {residual}")
    
    # 测试2：同时返回身份特征和残差
    print("\n" + "-" * 70)
    print("测试2: 同时返回身份特征和残差")
    print("-" * 70)
    with torch.no_grad():
        identity_features, residual = model(
            src=src,
    ...
    # 测试3：计算三元组距离（模拟三元组损失）
    print("\n" + "-" * 70)
    print("测试3: 计算三元组距离（模拟三元组损失）")
    print("-" * 70)
    with torch.no_grad():
        # 生成3个样本：anchor, positive, negative
        anchor = identity_features[0:1]  # [1, 512]
        positive = identity_features[1:2]  # [1, 512]
        negative = identity_features[2:3]  # [1, 512]
        ...
        print(f"正样本L2距离: {pos_dist.item():.4f}")
        print(f"负样本L2距离: {neg_dist.item():.4f}")
        print(f"正样本余弦相似度: {pos_sim.item():.4f}")
        print(f"负样本余弦相似度: {neg_sim.item():.4f}")
        print(f"三元组损失（margin=0.2）: {F.relu(pos_dist - neg_dist + 0.2).item():.4f}")
    ...
    triplet_params = sum(p.numel() for p in model.parameters())
    
    print(f"TransformerDecoderOnly3D (原始): {decoder3d_params:,}")
    print(f"TransformerDecoderOnly3D_Triplet (三元组): {triplet_params:,}")
    print(f"差异: {triplet_params - decoder3d_params:,} ({(triplet_params - decoder3d_params) / decoder3d_params * 100:.1f}%)")
