"""
模型1训练脚本：特征转换ControlNet
训练模型将姿势信息和特征信息转换为目标角度的特征，保持身份一致性
"""
import os
import sys
...
from train_transformer3D.controlnetlike.models_feature_controlnet import FeatureControlNet
...
def compute_identity_loss(pred_features, source_features, weight=0.3):
    """
    计算身份一致性损失：确保输出特征保持身份信息
    
    Args:
        pred_features: 预测特征 [batch, feature_dim]
    ...
    cosine_sim = (pred_norm * source_norm).sum(dim=1).mean()
    identity_loss = (1 - cosine_sim) * weight
...
def plot_training_curves(
    ...
    train_feature_losses = [m['feature_loss'] for m in train_metrics_history]
    train_identity_losses = [m['identity_loss'] for m in train_metrics_history]
    val_losses = [m['total_loss'] for m in val_metrics_history]
    val_feature_losses = [m['feature_loss'] for m in val_metrics_history]
    ...
    # 子图2: 特征损失
    ax2 = axes[0, 1]
    ax2.plot(epochs, train_feature_losses, label='训练特征损失', marker='o', linewidth=2, markersize=4)
    ax2.plot(epochs, val_feature_losses, label='验证特征损失', marker='s', linewidth=2, markersize=4)
    ax2.set_xlabel('Epoch', fontsize=12)
    ax2.set_ylabel('特征损失', fontsize=12)
    ax2.set_title('特征损失', fontsize=14, fontweight='bold')
    ax2.legend()
    ax2.grid(True, alpha=0.3)
    
    # 子图3: 身份损失
    ax3 = axes[1, 0]
    ax3.plot(epochs, train_identity_losses, label='训练身份损失', marker='o', linewidth=2, markersize=4)
    ax3.set_xlabel('Epoch', fontsize=12)
    ax3.set_ylabel('身份损失', fontsize=12)
    ax3.set_title('身份一致性损失', fontsize=14, fontweight='bold')
    ax3.legend()
    ax3.grid(True, alpha=0.3)
    
    # 子图4: 余弦相似度
    ax4 = axes[1, 1]
    ax4.plot(epochs, val_cosine_sims, label='验证余弦相似度', marker='^', linewidth=2, markersize=4, color='green')
    ax4.set_xlabel('Epoch', fontsize=12)
    ax4.set_ylabel('余弦相似度', fontsize=12)
    ax4.set_title('验证余弦相似度', fontsize=14, fontweight='bold')
    ax4.set_ylim([0, 1])
    ax4.legend()
    ax4.grid(True, alpha=0.3)
    
    plt.suptitle('特征转换ControlNet训练曲线', fontsize=16, fontweight='bold', y=1.02)
    ...
    if save_path:
        ...
        default_path = 'train_transformer3D/controlnetlike/feature_controlnet_training_curves.png'
...
def train_epoch(
    ...
    for batch_idx, batch in enumerate(pbar):
        ...
        source_pose = batch['source_pose'].to(device)
        ...
        if use_amp and scaler is not None:
            with autocast():
                ...
                # 计算身份一致性损失
                identity_loss = compute_identity_loss(
                    output_features, source_features, weight=identity_loss_weight
                )
                
                # 总损失
                total_batch_loss = feature_loss + identity_loss
        ...
        if writer is not None:
            ...
            writer.add_scalar('Train/FeatureLoss', feature_loss.item(), global_step)
    ...
    avg_feature_loss = total_feature_loss / num_batches if num_batches > 0 else 0.0
...
def validate(model, dataloader, criterion, device, use_amp=False, identity_loss_weight=0.3):
    """验证模型"""
    model.eval()
    total_loss = 0.0
    total_feature_loss = 0.0
    ...
    with torch.no_grad():
        for batch in tqdm(dataloader, desc='验证'):
            ...
            cosine_sim = (output_norm * target_norm).sum(dim=1).mean().item()
...
def main():
    parser = argparse.ArgumentParser(description='训练特征转换ControlNet')
    ...
    parser.add_argument('--hidden_dim', type=int, default=512,
                       help='隐藏层维度')
    parser.add_argument('--num_main_layers', type=int, default=3,
                       help='主网络层数')
    ...
    parser.add_argument('--weight_decay', type=float, default=1e-5,
                       help='权重衰减')
    parser.add_argument('--identity_loss_weight', type=float, default=0.3,
                       help='身份一致性损失权重')
    ...
    if args.loss_type == 'mse':
        ...
        from train_transformer3D.losses import CosineSimilarityLoss
        criterion = CosineSimilarityLoss()
    else:  # combined
        from train_transformer3D.losses import CombinedLoss
