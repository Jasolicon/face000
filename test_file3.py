"""
测试文件3 - 视频第一帧人脸检测、特征比对和中文标注（Decoder-Only版本）
使用 DINOv2 提取特征，与 features 目录中的特征对比（阈值0.25）
使用 TransformerDecoderOnly 模型进行特征矫正
在图片上标注关键点和角度
"""
import os
...
def process_video_frame(video_path, features_dir='features', output_path=None, similarity_threshold=0.25, 
    ...
    """
    处理视频指定帧：检测人脸、比对特征、标注结果（使用Decoder-Only模型）
    ...
    """
    print("=" * 70)
    print(f"视频第 {frame_number} 帧人脸检测和特征比对（Decoder-Only版本）")
    print("=" * 70)
    ...
    # 加载Transformer模型（Decoder-Only版本）
    transformer_model = None
    if transformer_model_path and Path(transformer_model_path).exists():
        print("\n正在加载Transformer模型（Decoder-Only版本）...")
        try:
            transformer_model, _ = load_transformer_model(
                transformer_model_path, 
                device=device,
                model_type='decoder_only'  # 明确指定使用decoder_only模型
            )
            print("  ✓ TransformerDecoderOnly模型已加载，将用于特征矫正")
        except Exception as e:
            ...
            try:
                transformer_model, _ = load_transformer_model(
                    transformer_model_path, 
                    device=device,
                    model_type=None  # 自动检测
                )
                print("  ✓ Transformer模型已加载（自动检测类型）")
            except Exception as e2:
                print(f"  ✗ 自动检测也失败: {e2}")
                print("  将跳过特征矫正步骤")
    else:
    ...
    # 提取每个人脸的特征并比对
    print("\n正在提取特征并比对...")
    print("  流程: InsightFace 裁剪的人脸区域 → DINOv2 提取特征 → TransformerDecoderOnly矫正 → 与DINO特征库比对")
    results = []
    matched_names = []
    matched_probs = []
    ...
    for i, face_info in enumerate(detected_faces):
        ...
        try:
            ...
            # 使用TransformerDecoderOnly矫正特征（如果模型已加载且提供了参考图像）
            if transformer_model is not None and reference_image_path and reference_landmarks_3d is not None:
                ...
                if hasattr(face_info, 'kps') and face_info.kps is not None:
                    ...
                    print(f"    ✓ 已使用TransformerDecoderOnly矫正特征（平均角度: {angles.mean():.2f}°）")
                else:
    ...
    if output_path:
        ...
        # 默认保存路径
        output_path = 'output_annotated_decoder_only.jpg'
        annotated_image.save(output_path)
        print(f"\n结果已保存到: {output_path}")
    
    print("\n" + "=" * 70)
    print("处理完成！（使用TransformerDecoderOnly模型）")
    print("=" * 70)
...
def main():
    ...
    # 输出路径
    output_path = 'video_frame_result_decoder_only.jpg'
    ...
    # Transformer模型路径（Decoder-Only版本）
    transformer_model_path = 'train_transformer/checkpoints/best_model_decoder-only.pth'  # Decoder-Only模型路径
    ...
    if annotated_image is not None:
        ...
        for i, result in enumerate(results, 1):
            ...
            if result['match']:
                print(f"  特征比对相似度: {result['similarity']:.4f} (与特征库的余弦相似度，已使用TransformerDecoderOnly矫正)")
